...
(Edit: I used to recommend Photoprism, but Immich ended up being a better fit with proper external library support.)

Once you have your photos pushed from Synchthing, you can use [Immich](https://immich.app/) to index them and run some machine learning magic to recognise faces, objects, colours, etc, almost putting Google Photos to shame. They don't have such a full featured app as Google Photos to replace as the main gallery app on Android, but having a full quality photo and video backup with a web viewer is already pretty good.

Make sure to follow the [installation docs](https://docs.immich.app/install/truenas/), as there are some tricky bits like having to set up datasets with Apps preset for data and Generic for Postgres.

### Ad blocking

Adguard Home

Needs router DNS redirected to it, so can take internet out if you have issues with your NAS, but otherwise very useful to block ads at the DNS level so phone apps are also protected.

### Voice assistant (Smart Home)

HomeAssistant

[Set up datasets](https://apps.truenas.com/resources/deploy-home-assistant/)

The important thing here too is to use Apps preset for data and media and Generic for Postgres.

Bit limited in the container comparing to full HAOS, but I prefer it for simplicity.
